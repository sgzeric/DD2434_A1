...
def IsoMap(K, array):
    ...
    dist_mat = np.ones([array.shape[0], array.shape[0]])*1000
    #construct the Adjacency matrix of graph G with k nearest neighbor of each point
    for i in range(array.shape[0]):
        temp = np.argpartition(array[i], K)[:K + 1]
        dist_mat[i][temp] = array[i][temp]
    '''
    ...
    '''
    graph = floyd_warshall(dist_mat)**2

    id_mat = np.mat(np.ones(graph.shape[0]))
    temp1 = graph*id_mat.T*id_mat/graph.shape[0]
    temp2 = np.array(id_mat.T*id_mat*graph/graph.shape[0])
    temp = id_mat.T*id_mat*graph*id_mat.T*id_mat/np.power(graph.shape[0], 2)
    B = -1/2*(graph-temp1-temp2+temp)
